// ReadAquarius_Command - This class initializes, checks, and runs the ReadAquarius() command.
...
/**
This class initializes, checks, and runs the ReadAquarius() command.
*/
public class ReadAquarius_Command extends AbstractCommand
...
/**
Data API for time series data requests.
*/
protected String _Corrected = "Corrected";
protected String _Raw = "Raw";
...
Constructor.
*/
public ReadAquarius_Command () {
	super();
	setCommandName ( "ReadAquarius" );
}
...
throws InvalidCommandParameterException {
    ...
    String DataApi = parameters.getValue ( "DataApi" );
	...
	if ( (DataApi != null) && !DataApi.equals("") &&
		!DataApi.equalsIgnoreCase(_Corrected) && !DataApi.equalsIgnoreCase(_Raw) ) {
        message = "The DataApi parameter value is invalid.";
		warning += "\n" + message;
           status.addToLog ( CommandPhaseType.INITIALIZATION,
               new CommandLogRecord(CommandStatusType.FAILURE,
                   message, "Specify " + _Corrected + " (default) or " + _Raw ) );
	}
	...
	*/
	...
	if ( Message.isDebugOn ) {
		Message.printStatus(2, routine, "LocationId=" + LocationId + " whereCount=" + whereCount + " readSingle=" + readSingle + " readMult=" + readMult);
 ...
 * @param ataApi the API to use for data reads ('Raw', or 'Corrected')
...
private HashMap<String,Object> createReadProperties (
	...
	if ( (dataApi != null) && !dataApi.isEmpty() ) {
		readProperties.put("DataApi", dataApi );
	}
...
public boolean editCommand ( JFrame parent ) {
	...
	return (new ReadAquarius_JDialog ( parent, this )).ok();
}
...
throws InvalidCommandParameterException, CommandWarningException, CommandException {
	...
	String LocationId = parameters.getValue("LocationId");
	if ( commandPhase == CommandPhaseType.RUN ) {
	    LocationId = TSCommandProcessorUtil.expandParameterValue(getCommandProcessor(), this, LocationId);
	}

	String DataApi = parameters.getValue ("DataApi" );
	if ( commandPhase == CommandPhaseType.RUN ) {
	    DataApi = TSCommandProcessorUtil.expandParameterValue(getCommandProcessor(), this, DataApi);
	}
    ...
    */
	...
	try {
		...
		if ( (DataStore != null) && !DataStore.equals("") ) {
		    ...
		    DataStore dataStore0 = ((TSCommandProcessor)getCommandProcessor()).getDataStoreForName( DataStore, AquariusDataStore.class );
	        if ( dataStore0 != null ) {
	            Message.printStatus(2, routine, "Selected datastore is \"" + dataStore0.getName() + "\"." );
				dataStore = (AquariusDataStore)dataStore0;
	        }
	    }
		if ( dataStore == null ) {
            message = "Cannot get Aquarius DataStore for \"" + DataStore + "\".";
            Message.printWarning ( 2, routine, message );
            status.addToLog ( commandPhase,
                new CommandLogRecord(CommandStatusType.FAILURE,
                    message, "Verify that a AquariusDataStore datastore is properly configured." ) );
			...
			if ( (LocationId != null) && !LocationId.isEmpty() ) {
				...
				if ( LocationId != null ) {
					tsident.setLocation(LocationId);
				}
				tsident.setSource("Aquarius");
				String dataType = "";
				...
				Message.printStatus ( 2, routine, "Reading a single Aquarius web service time series \"" + TSID + "\"" );
				TS ts = null;
				try {
				    ...
				    ts = null;
					message = "Unexpected error reading Aquarius time series \"" + TSID + "\" (" + e + ").";
					Message.printWarning ( 2, routine, message );
				...
				Message.printStatus(2, routine, "Reading multiple Aquarius time series using input filter.");
				...
				filterPanel = new Aquarius_TimeSeries_InputFilter_JPanel ( dataStore, 5 );
				...
				// - the Aquarius internal field is used to help users correlate the TSTool filter to Aquarius web services
				...
				// Make sure that size is set.
	       		if ( size == 0 ) {
					Message.printStatus ( 2, routine,"No Aquarius web service time series were found." );
			        // Warn if nothing was retrieved (can be overridden to ignore).
		            message = "No time series were read from the Aquarius web service.";
		            Message.printWarning ( warning_level,
				...
				for ( int i = 0; i < size; i++ ) {
					...
					tsidentString =
						...
						+ "." + interval;
		            // Update the progress.
					message = "Reading Aquarius web service time series " + (i + 1) + " of " + size + " \"" + tsidentString + "\"";
					...
					try {
						...
						message = "Unexpected error reading Aquarius web service time series \"" + tsidentString + "\" (" + e + ").";
						Message.printWarning ( 2, routine, message );
						Message.printWarning ( 2, routine, e );
						++warning_count;
	                    status.addToLog ( commandPhase,
        ...
        Message.printStatus ( 2, routine, "Read " + size + " Aquarius web service time series." );
        ...
        if ( commandPhase == CommandPhaseType.RUN ) {
            if ( tslist != null ) {
                ...
                if ( wc > 0 ) {
                    message = "Error post-processing Aquarius web service time series after read.";
                    Message.printWarning ( warning_level,
                ...
                if ( wc2 > 0 ) {
                    message = "Error adding Aquarius web service time series after read.";
                    Message.printWarning ( warning_level,
                        MessageUtil.formatMessageTag(command_tag,
                        ++warning_count), routine, message );
                        status.addToLog ( commandPhase,
		...
		Message.printWarning ( 3, routine, e );
		message ="Unexpected error reading time series from the Aquarius web service (" + e + ").";
		Message.printWarning ( warning_level,
		MessageUtil.formatMessageTag(command_tag, ++warning_count),
		routine, message );
        status.addToLog ( commandPhase,
